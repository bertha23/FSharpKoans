    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            match xs.Length<0 with
            |false->xs.Length
            |_->0  // write a function to find the length of a list
    ...
    let ``02 Finding the length of a list, the easy way`` () =
        let length (xs:'a list) : int=
            ...
            |false -> xs.Length
            |_ -> 0
        length [9;8;5;8;45] |> should equal 5
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            List.map f xs // write a map which applies f to each element
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
             List.fold (+) initialState xs // write a function to do what's described above
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            List.fold (*) initialState xs // write a function to multiply the elements of a list
